...
public:
	...
	std::string newWord(const int minLength, const int maxLength) {
		std::string word;
		do {
...
ExportedModel name = {
	...
	.models = {
		{
			modelData::value_type("aa", {0.0, 0.0}),
			modelData::value_type("ab", {0.0, 0.0}),
		},
		{
			modelData::value_type("aa", {0.0, 0.0}),
			modelData::value_type("ab", {0.0, 0.0}),
...
//*/
std::string model_to_literal(const WordGenerator &generator) {
	std::string s;
	const ExportedModel &model = generator.exportData();
	
	auto alphabetSize = model.alphabet.size();
	if (alphabetSize == 0 || model.models.size() == 0 || model.models[0].size() != alphabetSize) {
		return s;
	}
	// reserve
	size_t size = alphabetSize * 2 +
		10 * alphabetSize * model.models.size() * model.models[0].size();
	s.reserve(size);
	s += "ExportedModel name = {\n\t.alphabet = {";
	for (const char c : model.alphabet) {
		s += " '";
		s += c;
		s += "',";
	}
	...
	for (const modelData &data : model.models) {
		s += "\t\t{\n";
		// create individual models
		for (const modelData::value_type &pair : data) {
			s += "\t\t\tmodelData::value_type(\"";
			s += pair.first;
			s += "\", { ";
			// vector of weights
			for (const double d : pair.second) {
				s +=  std::to_string(d);
				s += ", ";
			}
			s += "}),\n";
